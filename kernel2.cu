...
void run_kernel2(const int8_t *filter, int32_t dimension, const int32_t *input,
 ...
 dim3 blocks((int) ceil(num_pixel / (float) num_thread));
  ...
  kernel2<<<blocks, threads>>> (cuda_filter, dimension, (const int32_t *) cuda_input, cuda_output, width, height);
  ...
  normalize2<<<blocks, threads>>> (cuda_output, width, height, global_min , global_max);
