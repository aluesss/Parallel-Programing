...
void run_kernel4(const int8_t *filter, int32_t dimension, const int32_t *input,
  ...
  cudaMemcpy(cuda_output, output, num_pixel * sizeof(int32_t), cudaMemcpyHostToDevice);
  ...
  kernel4<<<blocks, threads>>> (cuda_filter, dimension, (const int32_t *) cuda_input, cuda_output, width, height);
                        ...
                        int32_t height) 
{
  ...
  unsigned int gridSize = (blockDim.x * gridDim.x);//Use blockDim to replace blockSize
  while(idx < width * height){
    int row = idx / width;
    int column = idx % width;
    output[idx] = apply2d(filter, input, dimension, width, height, row, column);
    idx += gridSize;
  }
}
